...
class ewriter():
    def __init__(self):
        self.model = ChatOpenAI(model="gpt-4o", temperature=0) # gpt-3.5-turbo, gpt-4o
        self.PLAN_PROMPT = ("You are an expert writer tasked with writing a high level outline of a short 5 paragraph essay. "
        ...
        self.RESEARCH_PLAN_PROMPT = ("You are a researcher charged with providing information that can "
                                     ...
                                     "any relevant information. Only generate 10 queries max.")
        self.REFLECTION_PROMPT = ("You are a teacher grading an 5 paragraph essay submission. "
    ...
    def research_plan_node(self, state: AgentState):
        ...
        for q in queries.queries:
            response = self.tavily.search(query=q, max_results=10)
    ...
    def research_critique_node(self, state: AgentState):
        ...
        for q in queries.queries:
            response = self.tavily.search(query=q, max_results=5)
            for r in response['results']:
                content.append(r['content'])
        return {"content": content,
...
class writer_gui( ):
    ...
    def run_agent(self, start,topic,stop_after):
        ...
        if start:
            ...
            config = {'task': topic,"max_revisions": 5,"revision_number": 0,
